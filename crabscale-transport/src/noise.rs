...
impl NoiseResponder {
    ...
    pub fn respond(
        ...
        let server_ephemeral = StaticSecret::random();
...
impl StateWithAead {
    ...
    fn seal(self, cleartext: &mut [u8], tag: &mut [u8; AEAD_TAG_LEN]) -> State {
        ...
        self.state.mix_hash(cleartext).mix_hash(tag)
    }

    fn open(self, ciphertext: &mut [u8], tag: &[u8; AEAD_TAG_LEN]) -> Option<State> {
        let hash = self.state.hash;
        let state = self.state.mix_hash(ciphertext).mix_hash(tag);
        let nonce = Nonce::default();
        self.aead
...
mod tests {
    ...
    #[test]
    fn known_byte_vector_round_trip() {
        let server = NoiseResponder::from_bytes([0x42; 32]);
        let client_static = StaticSecret::from([0x24; 32]);
        let prologue = b"Tailscale Control Protocol v113";

        let (initiator, init_bytes) =
            NoiseInitiator::initialize(client_static, server.public_key(), prologue, 113);
        assert_eq!(init_bytes.len(), crate::messages::INIT_MESSAGE_LEN);

        let init = parse_init_message(&init_bytes).unwrap();
        let output = server.respond(&init, prologue).unwrap();
        assert_eq!(output.response.len(), crate::messages::RESPONSE_MESSAGE_LEN);
